...
const SolicitorsDiary = () => {
    ...
    const formatSlotData = (agencySolicitor) => {
        ...
        return {
            ...
            number: formatCode(agencySolicitor.number),
            ...
            created_by: agencySolicitor?.created_by ?? agencySolicitor?.updated_by,
            // Action buttons
            action: (
    ...
    const fetchSolicitors = async (page = 1) => {
        setLoading(true);
        try {
            const response = await api.get('/solicitors/index', {
